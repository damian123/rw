/*
 * Implementation of RWHermRFQREigServer
 *
 * Uses Lapack's root free QR algorithm to compute eigenvalues of
 * a symmetric matrix.
 *
 *
 * $Id: //lapack/13/rw/lapack/heigrf.cc#1 $
 *
...
template <class TypeT>
RWSymEigDecomp<double> RWHermRFQREigServer<TypeT>::decompose(const RWTriDiagDecomp<TypeT>& decomp)
{
...
template <class TypeT>
bool RWHermRFQREigServer<TypeT>::computeEigenVectors() const
{
    return false;
}
