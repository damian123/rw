 ...
 *
 *
 * $Id: //lapack/13/rw/lapack/hermct.cc#1 $
 *
...
RWHermMat<TypeT>::RWHermMat(const RWSymMat<double>& re)
    : vec(RWMathVec<TypeT>(RWConvertMathVec<double, TypeT>(re.dataVec())))
{
    n = re.cols();
}

template <class TypeT>
RWHermMat<TypeT>::RWHermMat(const RWSymMat<double>& re, const RWSkewMat<double>& im)
