/*
 * Implementation of RWSymQREigServer
 *
 * Uses Lapack's QR/QL algorithm to compute eigenvalues/vectors of
 * a tridiagonal matrix.
 *
 * $Id: //lapack/13/rw/lapack/seigqr.cc#1 $
 *
...
RWSymQREigServer<TypeT>::RWSymQREigServer()
{
  computeVecs_ = true;
}
*/
...
template <class TypeT>
RWSymQREigServer<TypeT>::RWSymQREigServer(bool vecs)
{
    computeVecs_ = vecs;
}

template <class TypeT>
RWSymEigDecomp<TypeT> RWSymQREigServer<TypeT>::decompose(const RWTriDiagDecomp<TypeT>& decomp)
{
    ...
    size_t zsize = computeVecs_ ? decomp.rows() : 0;
    RWGenMat<TypeT> Z(zsize, zsize, rwUninitialized);
    rw_lapack_int_t ldz = computeVecs_ ? n : 1;
    RWPRECONDITION(n >= 0);
    RWTWorkBuffer<TypeT> work(computeVecs_ ? static_cast<size_t>(2 * n + 1) : 0);
    ...
    RWSymQREigServer<TypeT>::n(eig) = decomp.rows();
...
template <class TypeT>
bool RWSymQREigServer<TypeT>::computeEigenVectors() const
{
    return computeVecs_;
}

template <class TypeT>
void RWSymQREigServer<TypeT>::computeEigenVectors(bool x)
{
    computeVecs_ = x;
}
