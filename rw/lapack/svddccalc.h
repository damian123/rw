#ifndef __RWSVDDCCALC_H__
#define __RWSVDDCCALC_H__

/****************************************************************************
* Class RWSVDDivConqCalc
*
...
* the right singular vectors of A.  The class uses a Divide and Conquer
* algorithm.
* $Id: //lapack/13/rw/lapack/svddccalc.h#1 $
*
 ...
 * @brief Computes a singular value decomposition of a matrix using
 * divide-and-conquer.
 *
 * Computes a singular value decomposition of a matrix A using
 * divide-and-conquer. Based on the LAPACK routine \c xgesdd. This class
 * may be used as the \c SVDCalc template parameter for the classes
 * \link RWSVDecomp RWSVDecomp<T,SVDCalc> \endlink,
 * \link RWSVServer RWSVServer<T,SVDCalc>
...
private:
    ...
    static bool callGesdd(char jobz,
                          RWGenMat<TypeT> A,
                          RWMathVec<norm_type>& sigma,
                          RWGenMat<TypeT>& U,
                          RWGenMat<TypeT>& VT);

    static bool callGesddCpxSpecl(char jobz,
                                  RWGenMat<TypeT> A,
                                  RWMathVec<norm_type>& sigma,
                                  RWGenMat<TypeT>& U,
                                  RWGenMat<TypeT>& VT);
...
public:
     ...
     *
     * @param A The input matrix for which the singular value decomposition is
     * being computed.
     *
     ...
     *
     * @param numLeftVectors The input number of left vectors to compute. If the
     * number is less than zero, the default number of vectors will be computed
     * (it is up to the developer to determine what the default is).
     *
...
#  include <rw/lapack/svddccalc.cc>
#endif

#endif // __RWSVDDCCALC_H__
