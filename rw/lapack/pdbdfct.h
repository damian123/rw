#ifndef __RWPDBDFCT_H__
#define __RWPDBDFCT_H__

/*
 * PDBandFact - A factorization of a positive definite band matrix
 *
 * $Id: //lapack/13/rw/lapack/pdbdfct.h#1 $
 *
 ...
 * matrices. See also \link RWPDFact RWPDFact<T>\endlink, and
...
private:
    ...
    typedef typename rw_linear_algebra_traits<TypeT>::banded_hermitian_type fact_matrix;
    ...
    double conditionSpecialized() const;
...
protected:
    ...
    void calldi(const fact_matrix& A, int job, ret_type* det, int* inert) const;
...
public:
     ...
     */
    RWPDBandFact();
    RWPDBandFact(const RWPDBandFact<TypeT>&);
     ...
     */
    RWPDBandFact(const fact_matrix& A, bool ec = true);
     ...
     * optional boolean parameter in the constructor, \link
     * RWPDBandFact(const fact_matrix&, bool ec) RWPDBandFact()\endlink, or
     * factor() to \c false, calling condition() generates an exception.
     */
     ...
     * satisfies \f$ Ax = b \f$, where \c A is the matrix represented by
     * this factorization. It is wise to call good() or fail() to make sure that
     * the factorization was successfully constructed.
     */
     ...
     * factorization. It is wise to call good() or fail() to make sure that the
     * factorization was successfully constructed.
     */
...
/**
 * @relates RWPDBandFact
 *
 * Solves a system of equations. Returns the vector \c x, which
 * satisfies \f$ Ax = b \f$, where \a A is the matrix represented by
 * the factorization. It is wise to call one of the member functions
 ...
 * RWPDBandFact(const fact_matrix&, bool ec) RWPDBandFact()\endlink constructor
 * or the factor() member function to \c false, calling \link
 * condition(const RWPDBandFact<TypeT>&) condition()\endlink generates an
 * exception.
 */
template <class TypeT>
inline typename rw_numeric_traits<TypeT>::norm_type  condition(const RWPDBandFact<TypeT>& A)
{
    return A.condition();
}
